...
| D48 | **Infrastructure modules may be `@Global()`; workflows and repositories may not** | Registering the tenant events publisher globally sits oddly beside the note on the routers module, which imports its dependencies explicitly *because* global modules hide their edges. The line is what the module is rather than where it lives: a publisher belongs with the cache and the database connection — every part of a service uses it, and none of it carries domain meaning, so an invisible edge costs nothing. A workflows or repositories module is the opposite, and making one ambient is precisely how `commonModules` came to carry messaging's repositories into a list called common. Both comments now say so, because a reader meeting the two rules a file apart would otherwise read them as an inconsistency |
| D47 | **A tenant's search alias is provisioned by an event, with lazy creation kept as the recovery path** | Creating a tenant in Identity publishes `identity.tenant-created.v1`; Messaging consumes it in its own group and calls `ensureAlias`, so a tenant's alias exists before its first message rather than being made on the write path. Verified end to end: an alias appeared about two seconds after the tenant was created, with no message ever sent. **This is the only dual write in the system** and it is a deliberate exception to ADR-002 — Identity stores the tenant, then publishes, and a crash between the two loses the event. That is tolerable only because the event is an optimisation rather than the mechanism: `ensureAlias` still creates the alias lazily, so a lost publish degrades to exactly the behaviour that existed before. Publishing therefore cannot fail the request either; it is logged and swallowed, because the tenant *was* created. What used to make the lost case dangerous — a write to a missing alias silently creating a concrete index with a dynamic mapping, after which the alias could never be created — is now refused outright by `action.auto_create_index: -messages-*`, verified against a live cluster. Messaging holds **its own copy** of the topic name and the payload shape: importing Identity's declaration would couple two contexts at compile time, which the lint boundary refuses, and duplication turns a producer change into a versioning problem, which is what it actually is |
| D46 | **Identity is a second bounded context with its own process, and its seeding routes say so in the path and are refused outside a local environment** | The brief asks for DDD and for multi-tenancy but never for tenants or users — yet `tenantId` was appearing out of a hand-signed token, which is a gap rather than a simplification. Identity now owns Tenant and User and issues the tokens messaging verifies; the runtime coupling between them is a signing key and an agreed token shape, and nothing else. Its endpoints live under `for-demo/` because they create tenants and hand out a token to whoever names a user, without checking a credential — there is no password, deliberately, since the exercise runs locally to demonstrate messaging. The path is a label, though, and a label has never stopped a request: `ForDemoOnlyGuard` refuses every one of them unless `APP_ENV` is local or test, and fails closed on an environment it does not recognise. Verified against a container started with `APP_ENV=prod` — health answered 200 and all three seeding routes answered 403. `commonModules` was split in the same change: it had carried messaging's repositories and use cases, so identity would have inherited them simply by starting up, and the lint boundary could not have caught it because the import named `@/messaging/app.module` |
| D45 | **The image has no default service; a container that names no command prints the choices and exits 1** | One image carries the API, the indexer and the migration runner (ADR-001), so defaulting to any one of them is arbitrary — and the arbitrary choice is dangerous rather than merely untidy. A `command:` forgotten on the indexer would start a second API instead: nothing crashes, nothing logs an error, and the only symptom is that search results quietly stop appearing. Failing at start costs seconds; that failure costs an afternoon. Removing the default alone was not enough, because the image's entrypoint is dumb-init and a bare `dumb-init --` prints `Usage: dumb-init [option] program [args]`, which reads as a broken base image rather than a missing argument. The default now lists the three commands on stderr and exits 1 — verified, including that the exit code really is 1 and that stdout stays empty |
| D44 | **One image, one command per service — and the `app` compose profile had never actually run** | Asked whether each service needs its own Dockerfile. It does not: the image is built once and every process overrides `command:`, which is ADR-001 applied to compose, and identity will join as another service rather than another build. Testing that claim rather than asserting it found three faults, none of them caused by the restructure that prompted the check. `docker compose run migrate` invoked `pnpm migrate:up`, and the production stage installs runtime dependencies only — there is no pnpm in it; the binary is invoked directly now, and not through `node`, since `.bin` entries are shell wrappers. The API container died at boot on `unable to determine transport target for "pino-pretty"`, because the logger chose the pretty transport whenever `APP_ENV !== prod` while `pino-pretty` is a devDependency absent from the image; it now asks whether the module resolves rather than which environment it is told it is in. And Kafka had no volume, so `docker compose down` took every topic and committed offset with it — after which the consumer correctly refuses to start (D38) on a topic that no longer exists. The API also needed `ELASTICSEARCH_NODE`, which it did not before search moved behind `MessageRepository`. Verified end to end afterwards: a message posted to the containerised API was searchable through it about three seconds later, the whole path running in containers |
...
- `CONTEXT-MAP.md` plus a glossary per bounded context
...
## 10. Tenant provisioning — closed

**This section described an open gap and now records how it was closed.** A `tenantId`
used to arrive in a hand-signed JWT with nothing behind it, and every tenant-shaped
resource was created lazily the first time something needed it.

Identity now owns the tenant lifecycle (D46). Creating a tenant publishes
`identity.tenant-created.v1`, and Messaging provisions the tenant's search alias from
it — before the tenant's first message rather than during it (D47). What follows is
the reasoning that led there, kept because the failure modes it describes are still
the ones to know about.

**The search alias was the piece that most wanted moving.** `MessageSearchIndex.ensureAlias`
...
Not built, and the reason changed once Identity arrived, so it is worth writing down
rather than re-deciding.

[ADR-007](./adr/007-contexts-in-one-deployable.md) declined a gateway because nothing
needed one: two local processes with no client between them. The testing UI planned
for a later phase is the first thing that does — a page in a browser talking to
Messaging and Identity is talking to two origins.

**But "one origin" is not the same requirement as "a gateway".** Enabling CORS on
both services is two lines in `setupApp` and solves exactly that. A gateway costs a
container, a config file, and a route table that repeats what both services' paths
are — a second place to keep in step, and the first place to look when a request
mysteriously 404s.

What would make a gateway worth its weight is something that genuinely belongs at
that layer and nowhere else: TLS termination, rate limiting, a single public port in
front of more than two services, or authentication terminated before the services
see it. This exercise has none of them.

**Decision: CORS when the UI lands; a gateway when there is a reason for one that is
not "the UI is on a different port".**
